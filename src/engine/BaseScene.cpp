...
Camera& BaseScene::getActiveCamera() { return activeCamera; }
void BaseScene::setActiveCamera(Camera& camera) { activeCamera = camera; }

RenderTarget* BaseScene::getRenderTarget() { return rt.get(); }
Texture2D* BaseScene::getRenderTargetAsTexture() { return rt->asTexture(); }
...
BaseScene::getAllGameObjects() {
    return gameObjectMap;
}

void BaseScene::addGameObject(std::shared_ptr<GameObject> go,
...
void BaseScene::addRenderableComponent(
    RenderableComponent* renderableComponent) {
    rcs.push_back(renderableComponent);
}

void BaseScene::cleanRenderableComponent() { rcs.clear(); }

void BaseScene::pause() { state = SC_PAUSED; }
...
SceneState BaseScene::getState() const { return state; }

BaseScene::~BaseScene() { Log("Scene destroyed"); }
}  // namespace Chronos