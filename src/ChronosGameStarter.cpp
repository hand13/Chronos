...
namespace Chronos{
    ...
    void ChronosGameStarter::show(){
        window->show();
    }
    void ChronosGameStarter::begin(){
        engine->begin();
    }
    ...
    void ChronosGameStarter::run(){
        while(engine->getState() != STOP){
            ...
            if(engine->getState() != STOP){
                ...
                if(tmpSize.width != eSize.width || tmpSize.height != eSize.height){
                    engine->changeSize(tmpSize);
                }
                engine->runInLoop();
                window->displayOffscreen(engine->getCurrentScene()->getRenderTargetTextureHandler());
                window->persent();
            }
        }
    }
