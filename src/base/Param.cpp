...
namespace Chronos {
    ...
    Param* constructParamFromType(const std::string& name,ParamType type,PackInfo packInfo,size_t size){
        Param * result = nullptr;
        #define CASE_NEW(X,Y,N,P) case(X):{result = new GenericParam<Y>(N,P);break;}
        switch (type) {
