...
void Logger::info(const std::string &msg) { _logger->information(msg); }
void Logger::debug(const std::string &msg) { _logger->debug(msg); }

void Logger::warning(const std::string &msg) { _logger->warning(msg); }
void Logger::fatal(const std::string &msg) { _logger->fatal(msg); }

Logger Logger::root() { return Logger(Poco::Logger::root()); }
...
static AutoPtr<Channel> createFileChannel() {
    Poco::AutoPtr<Poco::SimpleFileChannel> file(
        new Poco::SimpleFileChannel("test.log"));
