...
class GenericParam : public Param {
   ...
   public:
    ...
    virtual void* asData() override { return &value; }
    virtual std::string getName() override { return name; }
    virtual PackInfo getPackInfo() override { return packInfo; }
    T value;
    GenericParam(const std::string& name, const T& value, PackInfo packInfo)
        : name(name), value(value), packInfo(packInfo) {}
    GenericParam(const std::string& name, PackInfo packInfo)
        : name(name), packInfo(packInfo) {}
...
class GenericParam<RawData> : public Param {
   ...
   public:
    ...
    virtual void* asData() override { return value.getData(); }
    virtual std::string getName() override { return name; }
    virtual PackInfo getPackInfo() override { return packInfo; }
    RawData value;
    GenericParam(const RawData& value, const std::string& name,
                 PackInfo packInfo)
        : name(name), packInfo(packInfo), value(value) {}
    GenericParam(const std::string& name, size_t size, PackInfo packInfo)
        : name(name), packInfo(packInfo), value(size) {}
    virtual ~GenericParam(){};
};
