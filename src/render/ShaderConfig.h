...
namespace Chronos {
enum ConstantDataType { CDT_ConstantBuffer, CDT_Texture2D };
struct ConstantDataDef {
    std::string name;
    ConstantDataType type;
    size_t size;
    u8 slot;
    ConstantDataDef(const std::string& name, ConstantDataType type, size_t size,
                    u8 slot)
        : name(name), type(type), size(size), slot(slot) {}
};
class ShaderConfig {
   public:
