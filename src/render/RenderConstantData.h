...
class RenderConstantData {
   ...
   public:
    ...
    void setVertexProcData(const std::string& name, const T& value) {
        vertexProcDataMap.at(name).rawData->copyIntoThis((const u8*)&value, 0,
                                                         0, sizeof(T));
    }

    template <typename T>
    void setMaterialData(const std::string& name, const T& value) {
        materialDataMap.at(name).rawData->copyIntoThis((const u8*)&value, 0, 0,
                                                       sizeof(T));
    }
    ...
    inline const Texture2DMap& getTextureMap() const { return textureMap; }
    inline RawDataMap& getVertexProcDataMap() { return vertexProcDataMap; };
    inline RawDataMap& getMaterialDataMap() { return materialDataMap; }
    inline Texture2DMap& getTextureMap() { return textureMap; }
};
}  // namespace Chronos