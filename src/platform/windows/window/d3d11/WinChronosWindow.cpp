...
void WinChronosWindow::init(Renderer* renderer) {
    ...
    ::RegisterClassEx(&wc);
    hWnd =
        ::CreateWindow(wc.lpszClassName, TEXT("Chronos"), WS_OVERLAPPEDWINDOW,
                       100, 100, width, height, NULL, NULL, wc.hInstance, this);
    // Show the window
    createSwapChain();
...
void WinChronosWindow::resize(WPARAM wParam, unsigned int w, unsigned int h) {
    ...
    rtv->Release();
    HRESULT hr =
        swapChain->ResizeBuffers(2, 0, 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
    ThrowIfFailed(hr);
    createRenderTargetView();
...
void WinChronosWindow::draw() {
    ...
    deviceContext->IASetVertexBuffers(0, 1, verticesBuffer.GetAddressOf(),
                                      &stride, &offset);
...
void WinChronosWindow::persent() { draw(); }

void WinChronosWindow::createCanvasBuffer() {
    float vertices[] = {
        -1.f, -1.f, 0, 0, 1.f, 1.f,  1.f, 0, 1.f, 0.f, 1.f, -1.f, 0, 1.f, 1.f,

        -1.f, -1.f, 0, 0, 1.f, -1.f, 1.f, 0, 0,   0.f, 1.f, 1.f,  0, 1.f, 0.f,
    };
    ...
    sd.pSysMem = vertices;
    ThrowIfFailed(
        d3d11Device->CreateBuffer(&desc, &sd, verticesBuffer.GetAddressOf()));
}
void WinChronosWindow::createCanvaShader() {
    std::vector<unsigned char> vbuffer =
        readDataFromFile("resources/shader/d3d11/basic_vert.cso");
    ThrowIfFailed(d3d11Device->CreateVertexShader(
        vbuffer.data(), vbuffer.size(), NULL, vs.GetAddressOf()));

    std::vector<unsigned char> pbuffer =
        readDataFromFile("resources/shader/d3d11/basic_pixel.cso");
    ThrowIfFailed(d3d11Device->CreatePixelShader(pbuffer.data(), pbuffer.size(),
                                                 NULL, ps.GetAddressOf()));
    const D3D11_INPUT_ELEMENT_DESC basicVertexLayoutDesc[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
         D3D11_INPUT_PER_VERTEX_DATA, 0},
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 3 * sizeof(float),
         D3D11_INPUT_PER_VERTEX_DATA, 0}};

    ThrowIfFailed(d3d11Device->CreateInputLayout(
        basicVertexLayoutDesc, ARRAYSIZE(basicVertexLayoutDesc), vbuffer.data(),
...
void WinChronosWindow::createRenderTargetView() {
    ...
    HRESULT hr = swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D),
                                      (LPVOID*)&pBackBuffer);
    ThrowIfFailed(hr);
    hr = d3d11Device->CreateRenderTargetView(pBackBuffer, NULL,
                                             rtv.GetAddressOf());
    ThrowIfFailed(hr);
    pBackBuffer->Release();
}
...
void WinChronosWindow::createSwapChain() {
    ...
    ThrowIfFailed(d3d11Device->QueryInterface(__uuidof(IDXGIDevice),
                                              (void**)&pDXGIDevice));
    IDXGIAdapter* pDXGIAdapter = nullptr;
    ThrowIfFailed(pDXGIDevice->GetAdapter(&pDXGIAdapter));
    pDXGIAdapter->GetParent(__uuidof(IDXGIFactory), (void**)&factory);

    ThrowIfFailed(factory->CreateSwapChain(d3d11Device.Get(), &sd,
    ...
    ThrowIfFailed(d3d11Device->CreateSamplerState(&samplerDesc, &sampler));
    this->sampler = sampler;
}

void WinChronosWindow::quit() { finished = true; }

WinChronosWindow::~WinChronosWindow() { Log(TEXT("window destructed")); }
