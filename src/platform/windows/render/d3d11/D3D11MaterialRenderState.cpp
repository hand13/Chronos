...
namespace Chronos {
    ...
    D3D11MaterialRenderState::D3D11MaterialRenderState(D3D11Renderer * renderer,Material * material){
    ...
    void D3D11MaterialRenderState::update(){
        ...
        ps = std::dynamic_pointer_cast<ChronosPixelShader>(rl->loadShader(sc->getShaderName(),
            sc->getShaderType(),true));
    ...
    void D3D11MaterialRenderState::createConstantBufferAndTextures(){
        ...
        for(auto & def : sc->getConstantDataDef()){
            if(def.type == CDT_Texture2D){
                ...
                pixelConstantBuffers[def.slot] = renderer->createConstantBuffer(D3D11Renderer::span16(static_cast<u16>(def.size)));
            }
        }
    }

    void D3D11MaterialRenderState::updateConstantBuffer(const RenderConstantData& rcd){
    ...
    void D3D11MaterialRenderState::updateTextures(const RenderConstantData& rcd){
        ...
        for(auto & st : tm){
            ...
            textures[st.second.slot] = std::dynamic_pointer_cast<ChronosD3D11Texture2D>(st.second.texture);
        }
    }
