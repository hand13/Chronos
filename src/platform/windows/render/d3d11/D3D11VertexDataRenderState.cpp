...
namespace Chronos {
D3D11VertexDataRenderState::D3D11VertexDataRenderState(D3D11Renderer* render,
                                                       VertexData* robj) {
    this->render = render;
    this->robj = robj;
}
...
void D3D11VertexDataRenderState::update() {
    {
        ...
        ThrowIfFailed(render->getDevice()->CreateBuffer(
            &desc, &sd, verticeBuffer.GetAddressOf()));
    }
    ...
    if (robj->getIndices().size() != 0) {
        ...
        ThrowIfFailed(render->getDevice()->CreateBuffer(
            &desc, &sd, indicesBuffer.GetAddressOf()));
    }
    dirty = false;
}
...
void D3D11VertexDataRenderState::applyWith(const RenderConstantData& rcd) {
    ...
    dc->IASetPrimitiveTopology(
        static_cast<D3D11_PRIMITIVE_TOPOLOGY>(robj->drawTopologyType));
    dc->IASetVertexBuffers(0, 1, verticeBuffer.GetAddressOf(), &stride,
                           &offset);
    if (indicesBuffer) {
