...
#include <vector>
namespace Chronos{
    D3D11VertexProcRenderState::D3D11VertexProcRenderState(D3D11Renderer * renderer,VertexProc * vertexProc,Geometry::AttributeSet * as){
    ...
    void D3D11VertexProcRenderState::update(){
        ...
        std::vector<D3D11_INPUT_ELEMENT_DESC> eleDesc = D3D11Renderer::genInputElementDescFromAttrSet(as);
        vs =std::dynamic_pointer_cast<ChronosVertexShader>(rl->loadShader(sc->getShaderName(), sc->getShaderType(),true
        , eleDesc.data(),eleDesc.size() * sizeof(D3D11_INPUT_ELEMENT_DESC)));
    ...
    void D3D11VertexProcRenderState::createConstantBuffer(){
        ...
        for(auto & def : sc->getConstantDataDef()){
            vertexProcConstantBuffers[def.slot] = renderer->createConstantBuffer(D3D11Renderer::span16(static_cast<u16>(def.size)));
        }
    }
    void D3D11VertexProcRenderState::updateConstantBuffer(const RenderConstantData& rcd){
        ID3D11DeviceContext * dc = renderer->getDeviceContext();
        ...
        for(auto & cd: rdm){
            ...
            if(vertexProcConstantBuffers.find(slot) == vertexProcConstantBuffers.end()){
                Panic("should not happended");
            }
            ...
            dc->UpdateSubresource(cb, 0, 0, rd->getConstantData(),static_cast<UINT>(rd->getSize()), 0);
        }
    }
