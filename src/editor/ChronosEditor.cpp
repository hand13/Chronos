...
void ChronosEditor::setScene(Scene* s) { currentScene = s; }
...
void ChronosEditor::processMsg(const MSG& msg) { eventBridge.pushMSG(msg); }
...
void ChronosEditor::showEditorArea() {
    ...
    // Right
    {
        ...
        ImGui::BeginChild("Main View",
                          ImVec2(static_cast<float>(size.width),
                                 0));  // Leave room for 1 line below us
        ImGui::Separator();
...
void ChronosEditor::showRenderView() {
    ...
    ImGui::Image(srv, ImVec2(static_cast<float>(size.width),
...
void ChronosEditor::showComponentDetailView() {
    if (selectedComponent) {
        ...
        if (klass) {
            showObject(selectedComponent, klass,
                       selectedComponent->getName().c_str());
        }
    }
}
...
ChronosEditor::~ChronosEditor() { logger.info("editor destructed"); }
...
void ChronosEditor::showFloat2(Float2& f2, boolean editable,
                               const char* label) {
    ImGui::InputFloat2(label, (float*)&f2);
}
void ChronosEditor::showFloat3(Float3& f3, boolean editable,
                               const char* label) {
    ImGui::InputFloat3(label, (float*)&f3);
}

void ChronosEditor::showFloat4(Float4& f4, boolean editable,
                               const char* label) {
    ImGui::InputFloat4(label, (float*)&f4);
}

void ChronosEditor::showPrimaryObject(void* object, const Klass* klass,
                                      const char* name) {
...
bool ChronosEditor::showSpecialObject(void* object, const Klass* klass,
                                      const char* name) {
...
void ChronosEditor::showObject(void* object, const Klass* klass,
                               const char* name) {
    ...
    for (const Field& f : klass->fields) {
        if (f.valueType.klass != nullptr) {
            if (showSpecialObject(f.getPointer<void>(object), f.valueType.klass,
                                  f.name.c_str())) {
                continue;
            }

            showObject(f.getPointer<void>(object), f.valueType.klass,
                       f.name.c_str());
        }
    }
}
