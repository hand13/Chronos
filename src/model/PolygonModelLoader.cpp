...
std::shared_ptr<PolygonModel> PolygonModelLoader::loadPolygonModel(
...
void PolygonModelLoader::processNode(aiNode* node, const aiScene* scene,
                                     const std::string& dir,
                                     std::shared_ptr<PolygonModel>& target) {
...
void PolygonModelLoader::processMesh(aiMesh* mesh, const aiScene* scene,
                                     const std::string& dir,
                                     std::shared_ptr<PolygonModel>& target) {
    ...
    size_t vertCompoentCount =
        as.totalSize() / sizeof(float);  // 一个顶点有多少个float数据
    ...
    if (mesh->HasPositions()) {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
            unsigned int offset =
                static_cast<unsigned int>(i * vertCompoentCount + attr_offset);
