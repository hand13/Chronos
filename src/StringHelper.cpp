...
std::string WideToUTF8(const std::wstring& str) {
    ...
    ucnv_fromUChars(convert,tmp,static_cast<unsigned int>(sizeof(char) * buffer_size)
        ,t,static_cast<unsigned int>(str.size()),&code);
    if(U_FAILURE(code)){
        Panic("should not happended");
    }
...
std::wstring UTF8toWide(const std::string& str){
    ...
    ucnv_toUChars(convert,tmp,static_cast<unsigned int>(sizeof(UChar) * buffer_size)
