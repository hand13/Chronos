...
BaseGameObject::BaseGameObject(Scene* scene) {
    root = nullptr;
    this->scene = scene;
    attachComponentTo(std::make_shared<DefaultRootComponent>(this), nullptr,
                      "root");
    auto tc = std::make_shared<TransformComponent>(this);
    transformComponent = tc.get();
    attachComponentTo(tc, root, "transform");
}
void BaseGameObject::attachComponentTo(std::shared_ptr<Component> component,
                                       Component* target,
...
Component* BaseGameObject::getRootComponent() { return root; }
...
Scene* BaseGameObject::getScene() { return scene; }
...
BaseGameObject::~BaseGameObject() { Log("GameObject destructed"); }
}  // namespace Chronos