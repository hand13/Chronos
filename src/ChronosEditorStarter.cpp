...
namespace Chronos{
    ...
    void ChronosEditorStarter::begin(){
        engine->begin();
    }

    void ChronosEditorStarter::init(ChronosEngine * e,Renderer* r,ChronosEditor * ed){
        engine = e;
        renderer = r;
        editor = ed;
    ...
    void ChronosEditorStarter::run(){
        while(!editor->isStop()){
            ...
            if(!editor->isStop()){
                ...
                editor->displayOffscreen(engine->getCurrentScene()->getRenderTargetTextureHandler());
                editor->runInLoop();
