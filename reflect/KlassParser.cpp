...
KlassParser::KlassParser() { index = clang_createIndex(0, 0); }

KlassParser::~KlassParser() { clang_disposeIndex(index); }
...
void KlassParser::parseFileIntoParseContext(
    ...
    std::vector<const char*> commands(
        include_dirs.size() + sizeof(command_line_args) / sizeof(char*));
    // commands.reserve(include_dirs.size() +
    // sizeof(command_line_args)/sizeof(char*));
    ...
    CXTranslationUnit unit = clang_parseTranslationUnit(
        index, file_name, commands.data(), static_cast<int>(commands.size()),
        ...
        [](CXCursor c, CXCursor parent, CXClientData clientData) {
            ...
            switch (c.kind) {
                ...
                case CXCursor_Constructor: {
                    ...
                    constructor->access =
                        fromCXSpecifierToAccess(clang_getCXXAccessSpecifier(c));
                    KlassInfo* klass =
                        dynamic_cast<KlassInfo*>(pc->stack.top());
                    if (klass == nullptr) {
                        throw std::exception("should not happended");
                    }
                ...
                case CXCursor_ParmDecl: {
                    ...
                    MethodInfo* method =
                        dynamic_cast<MethodInfo*>(pc->stack.top());
                    ConstructorInfo* constructor =
                        dynamic_cast<ConstructorInfo*>(pc->stack.top());
